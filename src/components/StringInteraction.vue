  ...
  >
    <v-card-text>
      ...
      <v-row justify="space-between">
        <div style="width: 300px" class="ml-6">
          <v-text-field
            variant="underlined"
            label="Search"
            v-model="serachBox"
        ...
        <v-btn class="mt-6" size="x-small" color="indigo" @click="search()" rounded="lg"
          >Search Char</v-btn
        >
      </v-row>
    </v-card-text>
  </v-card>
  <v-snackbar color="red" location="top" timeout="2000" v-model="snackVal">{{
    errorMessage
  }}</v-snackbar>
</template>
...
const search = () => {
  if (textBox.value != "") {
    if (textBox.value.includes(serachBox.value)) result.value = "Char found"
    if (!textBox.value.includes(serachBox.value)) result.value = "Char not found"
  } else {
    snackVal.value = true
  }
}
