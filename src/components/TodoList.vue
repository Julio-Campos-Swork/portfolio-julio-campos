<template>
  <v-card>
    <v-card-title class="text-center text-h6 text-purple">Todo List</v-card-title>
    <v-card-text>
      <p class="text-center ">Enter a task</p><br>
      <v-row justify="space-evenly">
      <v-text-field v-model="inputLabel" clearable variant="solo" @keypress.enter="newTask()"  style="width: 250px;" ></v-text-field>
      <v-btn class="mt-4 ml-2" color="green" elevation="12" @click="newTask()">Add New Task</v-btn>
      </v-row>
      <v-row justify="center">
        <p class="ml-8"> <v-btn variant="text" color="indigo" @click="showAll()">Pending Task</v-btn> </p>
        <p class="ml-8"> <v-btn variant="text" color="green" @click="showCompleted()">Completed</v-btn> </p>
        <p class="ml-8"> <v-btn variant="text" color="red" @click="showDeleted()">Deleted</v-btn> </p>
      </v-row>

      <v-col v-if="alltask" cols="12" v-for="item in items.items" :key="item.id">
        <v-row class="mt-2" justify="space-between">
        <v-text-field variant="underlined" v-model="item.task" class="text-indigo ml-4" @keypress.enter="editTask(item.task, item.id)">{{item.id}} -</v-text-field>



          <!-- botones -->
        <div class="mr-6">
        <v-btn variant="text" color="indigo" size="small"  @click="editTask(item.task, item.id)">
      ...
      >Edit</v-tooltip>
        ...
        <v-btn color="green" variant="text" size="small" @click="completedItem(item.id)">
      ...
      >Delete</v-tooltip>
        ...
        <!-- completed task -->
        <v-col v-if="completedTask" cols="12" v-for="item in itemsCompleted.itemsCompleted" :key="item.id">
        <v-row class="mt-2" justify="space-between">
      ...
      <v-row v-if="completedTask" class="mt-6 ml-4" justify="start" >
        ...
        <v-btn v-if="completedTask" color="red" size="small" @click="localStorage.removeItem('completed-items')">Clean Completed</v-btn>
      </v-row>


<!-- deleted task -->
        <v-col v-if="deletedTask" cols="12" v-for="item in itemsDeleted.itemsDeleted" :key="item.id">
        <v-row class="mt-2" justify="space-between">
        <h6 class="text-red text-h5 ml-4"> {{ item.task }}</h6>
        </v-row>
      </v-col>
      <v-row v-if="deletedTask" class="mt-6 ml-4" justify="start" >
        <v-btn size="small" rounded="lg" color="red" @click="localStorage.removeItem('deleted-items')">Clean Deleted</v-btn>
      </v-row>


      <v-row class="mt-10" justify="end">

      <v-btn color="red" @click="removeAll()" elevation="12" rounded="lg" size="x-small">Eliminar todo</v-btn>
      </v-row>
    </v-card-text>
  </v-card>
  <v-snackbar v-model="snakAlert" timeout="3000" color="red" location="top">The task already exist in task list</v-snackbar>
</template>
