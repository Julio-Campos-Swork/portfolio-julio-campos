<template>
  <v-card elevation="24" rounded="xl" class="mt-4 mb-4" >
    <v-card-text class="text-center">
      ...
      <p class="text-red-accent-1 text-h5">ID: <span class="text-green-lighten-3 text-h6"> {{ id }}</span></p>
      <p class="text-red-accent-1 text-h5">Username: <span class="text-green-lighten-3 text-h6"> {{ username }}</span></p>
      <p class="text-red-accent-1 text-h5">Email: <span class="text-green-lighten-3 text-h6"> {{ email }}</span></p>
      <p class="text-red-accent-1 text-h5">Gender: <span class="text-green-lighten-3 text-h6"> {{ gender }}</span></p>
      <p class="text-red-accent-1 text-h5">Birthday: <span class="text-green-lighten-3 text-h6"> {{ birthday }}</span></p>
      <br>

      <v-btn @click="getUser()" color="blue" rounded="lg" elevation="12" class="mt-4">Generate random user</v-btn><br>
      <v-btn @click="reset()" color="red" rounded="lg" elevation="12" class="mt-4" v-if="username">Reset</v-btn>
    </v-card-text>
  </v-card>
</template>
