  ...
  >
    <v-card-text>
      <v-row justify="space-around" class="mr-2 mt-6 ml-2">
        <v-label class="text-black text-caption mr-4">Choose your Age: </v-label>
      ...
      <v-row justify="center" class="mt-8">
        <v-btn color="indigo" rounded="lg" size="x-small" @click="calculate()"
          >Caculate</v-btn
        >
      </v-row>

      <p class="text-center mb-8 mt-8">Your Age is:</p>
      <v-row justify="space-around" class="mt-4 mb-10">
        <v-label class="text-black text-body-2">Year(s)</v-label>
        <v-label class="text-black text-body-2">Month(s)</v-label>
        <v-label class="text-black text-body-2">Day(s)</v-label>
      </v-row>
      <v-row justify="space-around" class="mt-4 mb-4">
        <v-btn size="small" class="rounded-xl text-white bg-black">{{ birthYear }}</v-btn>
        <v-btn size="small" class="rounded-xl text-white bg-black">{{
          birthMonth
        }}</v-btn>
        <v-btn size="small" class="rounded-xl text-white bg-black">{{ birthDate }}</v-btn>
      </v-row>
      <v-row justify="center">
        <v-btn class="mt-10 mb-4" color="red" size="x-small" rounded="lg" @click="init()"
          >Reset</v-btn
        >
      </v-row>
    </v-card-text>
